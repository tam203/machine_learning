...
#  We can experiment with different tree depths, this can lead to overfitting the data.
# battleTree = tree.makeTree(knightStats, battleResults, 100)

# Remind our selfs which index is which skill
...
for s,r in zip(knightStats, battleResults):
    p = tree.applyTree(battleTree, s)
    correct.append(1-abs(r - p))
print "`testting` against my test data I got %s out of %s correct, that's %s%%." % (sum(correct), len(correct), ((sum(correct)*100.0)/len(correct)))
